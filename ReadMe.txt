...
Battlefield 2 Demo on Windows XP SP3 (32 bit with English language) has been quickly tested but Battlefield 2 Complete Collection on a powerful computer with Windows 10 + some vehicles from Mod DB (https://www.moddb.com/games/battlefield-2/addons) are recommended.
...
_ Note: See Known issues section below.
...
_ Some custom vehicles from the Internet sometimes have problems, check the comments column in "vehicles_db.csv" for some known issues and possible solutions, as well as "Object_patches" folder (use e.g. WinMerge to compare with the original version to see the changes).
